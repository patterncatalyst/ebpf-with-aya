#!/usr/bin/env bash
#
# scripts/lab/provision-vm.sh — provision a Fedora 44 KVM guest via cloud-init.
#
#   ./provision-vm.sh ebpf-target        # the eBPF deploy target
#   ./provision-vm.sh ebpf-peer          # optional second host for 2-host tests
#
# Idempotent-ish: refuses to clobber an existing domain of the same name.
# Run ./destroy-vm.sh <name> first if you want a clean rebuild.
#
# Requires (host): virt-install, qemu-img, cloud-localds, libvirtd running,
# your user in the `libvirt` group. See Chapter 1.
#
# Downloads the Fedora 44 Cloud Base qcow2 once into ~/.cache, then layers a
# per-VM overlay disk so re-provisioning is cheap and the base stays pristine.

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)" && cd "$SCRIPT_DIR"

# ---- Config -----------------------------------------------------------------
VM_NAME="${1:?usage: provision-vm.sh <vm-name>  (e.g. ebpf-target)}"
VCPUS="${VCPUS:-2}"
RAM_MB="${RAM_MB:-4096}"
DISK_GB="${DISK_GB:-20}"
...
echo "and connect:"
echo "    ssh fedora@\$(./vm-ip.sh $VM_NAME)"
