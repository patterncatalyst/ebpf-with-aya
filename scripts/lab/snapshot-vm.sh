...
#   ./snapshot-vm.sh ebpf-target                 # snapshot named "lab-ready"
#   ./snapshot-vm.sh ebpf-target clean-updated   # a differently-named snapshot
#
# Take a snapshot after the guest is fully provisioned + tooled (cloud-init done,
# libbpf-devel/gdbserver/JDK installed) so you can revert to a known-good state in
# seconds instead of re-provisioning. Snapshots of a running guest capture RAM +
# disk, so a revert restores it running.
#
# This host uses the system libvirt instance; the URI is forced below. If your
# login shell isn't in the `libvirt` group yet, run via: sg libvirt -c '...'.
set -euo pipefail
