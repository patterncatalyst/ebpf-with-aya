...
#
# Starts ebpf-target and ebpf-peer if they're defined but not running, then waits
...
"$SCRIPT_DIR/vm-ip.sh" "$TARGET" >/dev/null 2>&1 && echo "ebpf-target: $("$SCRIPT_DIR/vm-ip.sh" "$TARGET")" || echo "ebpf-target: no lease yet"
"$SCRIPT_DIR/vm-ip.sh" "$PEER"   >/dev/null 2>&1 && echo "ebpf-peer:   $("$SCRIPT_DIR/vm-ip.sh" "$PEER")"   || echo "ebpf-peer:   no lease / not provisioned"
