...
#
# Demos attach a probe and then drive a workload on the VM(s) in the background
# (nohup load-generators, ncat listeners, churn loops). Those outlive the demo's
# foreground process, so a plain Ctrl-C leaves them running and pinning the VM's
# CPU. Source this and call `reap <ssh-target> <pattern>` once per background
# workload the demo launches; on exit (normal or Ctrl-C) every registered
# pattern is killed on its host.
#
