...
OUT = "ebpf-with-aya/assets/diagrams"
...
def _svg(width, height, bands, nodes, edges, notes):
    ...
    for t in notes:
        ...
        o.append(f'<text x="{t["x"]}" y="{t["y"]}" font-size="{t.get("size",11)}" fill="{t.get("color","#888")}" text-anchor="{anchor}"{bold}>{esc(t["text"])}</text>')
    o.append('</svg>')
    return "\n".join(o)
...
def _exc(bands, nodes, edges, notes):
    ...
    for b in bands:
        ...
        if b.get("label"): text(b["x"]+12,b["y"]+6,b["label"],12,"#888888")
    for n in nodes:
        fill,stroke = STYLES[n.get("style","box")]
    ...
    for t in notes:
        text(t["x"],t["y"]-10,t["text"],t.get("size",11),t.get("color","#888888"))
