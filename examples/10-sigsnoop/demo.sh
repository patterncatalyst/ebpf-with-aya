#!/usr/bin/env bash
# examples/10-sigsnoop/demo.sh — build, deploy, run sigsnoop on the target VM.
#   ./demo.sh   |   ./demo.sh build   |   VM=ebpf-target ./demo.sh
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)" && cd "$SCRIPT_DIR"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"; LAB="$REPO_ROOT/scripts/lab"
VM="${VM:-ebpf-target}"; BIN="$SCRIPT_DIR/target/release/sigsnoop"
...
build(){ c_step "building sigsnoop (release)"; cargo build --release || c_fail "cargo build failed"; [[ -x "$BIN" ]] || c_fail "no binary at $BIN"; c_ok "built $BIN"; }
case "${1:-run}" in
  build) build ;;
  run|*)
    build
    ...
    c_info "generate signals on the target, e.g.:  ssh fedora@$IP 'sleep 60 & p=\$!; kill -TERM \$p; kill -0 \$p 2>/dev/null || true'"
