...
$SSH "fedora@$TIP" 'cd /tmp/cc && sudo rm -f /sys/fs/bpf/bpf_reno 2>/dev/null; sudo bpftool struct_ops register cc.o /sys/fs/bpf/bpf_reno 2>&1 && echo registered || echo "register failed — kernel may lack bpf struct_ops CC support"'
...
$SSH "fedora@$TIP" 'sudo rm -f /sys/fs/bpf/bpf_reno; echo unregistered'
