#!/usr/bin/env bash
#
# examples/08-fentry-unlink/demo.sh
#
# Build fentrysnoop on the host, deploy to the target VM, run it, generate
# unlink traffic (including a guaranteed failure so you see a non-zero ret),
# and point you at Grafana.
#
...
VM="${VM:-ebpf-target}"
BIN="$SCRIPT_DIR/target/release/fentrysnoop"
...
build() {
  c_step "building fentrysnoop (release) on the host"
...
case "${1:-run}" in
  ...
  run|*)
    ...
    c_info "in another terminal, generate successful AND failing unlinks:"
    c_info "    ssh fedora@$IP 'for i in \$(seq 1 10); do t=\$(mktemp); rm -f \"\$t\"; done; rm -f /nonexistent-\$RANDOM 2>/dev/null || true'"
